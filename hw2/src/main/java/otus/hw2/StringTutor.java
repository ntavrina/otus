...
public class StringTutor  {
	...
	public boolean checkGreeting(String greeting) {
		if(greeting == null || greeting.isEmpty()) return false;

		if(!greeting.startsWith("Привет")) return false;
		if(!greeting.endsWith("!")) return false;

		String fname = greeting.substring(greeting.lastIndexOf(",") + 1, greeting.indexOf("!")).trim();
		String[] FN = fname.split(" ");

		if(FN.length > 0 && FN[0].length() < 3) return false;
		if(FN.length > 1 && FN[1].length() < 3) return false;
		if(FN.length < 2) return false;
		if(!FN[0].toUpperCase().substring(0,1).equals(FN[0].substring(0,1))) return false;
		if(!FN[1].toUpperCase().substring(0,1).equals(FN[1].substring(0,1))) return false;
